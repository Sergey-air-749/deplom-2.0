...
export default function ChangeEmail() {
    ...
    const submitUserUpData = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault()
            ...
            try {
                ...
                const response = await axios.post('http://localhost:7000/api/change/email/verify',
                    codeObj,
    ...
    const buttonGetТewСode = async () => {
            ...
            try {
                ...
                const response = await axios.get('http://localhost:7000/api/change/email/new',
                    {
                        headers: {
                            'authorization': `Bearer ${token}`,
                            'Content-Type': 'application/json'
    ...
    const buttonBackPage = async () => {
            ...
            try {
                ...
                const response = await axios.get('http://localhost:7000/api/change/email/cancel',
                    {
                        headers: {
                            'authorization': `Bearer ${token}`,
                            'Content-Type': 'application/json'
                ...
                <div className={style.formHead}>
                    ...
                    <div className={style.formTitle}>
...

                        {
                            userData != null ? (
                                <div >
                                    <p>Мы отправели код потверждения на <span>{userData?.emailNew}</span></p>    
                                </div>
                            ) : (
                                <div></div>
                            )
